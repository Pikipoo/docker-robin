...
RUN sudo docker pull postgres:9.5.7 && \
    sudo docker run --name robin-postgres -e POSTGRES_PASSWORD=root -d postgres:9.5.7 && \
    sudo docker exec -it postgres-robin psql -U postgres -d robin -c 'CREATE TABLE robin_base(ip INET PRIMARY KEY, total_probability REAL NOT NULL DEFAULT 0);'
